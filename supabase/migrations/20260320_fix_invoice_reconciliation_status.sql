...
BEGIN
    RAISE NOTICE 'Iniciando Reconciliación V5.3.2 (Propagación de Estado Conciliado)';
    ...
    LOOP
        ...
        WHERE c.organization_id = p_org_id 
          ...
          AND (
              (v_mov.tipo = 'cobro' AND c.tipo IN ('factura_venta', 'nota_debito_venta', 'ingreso_vario')) 
              OR 
              (v_mov.tipo = 'pago' AND c.tipo IN ('factura_compra', 'nota_debito_compra', 'egreso_vario'))
          )
          AND (
    ...
    LOOP
        ...
        IF v_match_id IS NOT NULL THEN
            ...
            IF NOT p_dry_run THEN
                ...
                -- NUEVO: Propagación del estado conciliado a los comprobantes aplicados
                UPDATE public.comprobantes
                SET estado = 'conciliado',
                    metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
                        'last_auto_reconciled', now(),
                        'transaccion_bancaria_id', v_trans.id
                    )
                WHERE id IN (
                ...
                AND (monto_pendiente <= 0.05 OR estado = 'pagado');
            END IF;
        ELSE
            IF NOT p_dry_run THEN
                UPDATE public.transacciones 
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Error Crítico V5.3.2: %', SQLERRM;
