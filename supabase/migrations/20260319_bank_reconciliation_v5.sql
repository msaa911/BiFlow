...
BEGIN
    ...
    LOOP
        ...
        -- Match si el entero es el mismo (ROUND 0)
        ...
        IF v_inv_match.id IS NOT NULL THEN
            ...
            RAISE NOTICE 'Fase 1 Match: Mov % -> Factura %', v_mov.id, v_inv_match.id;
        ...
        ELSE
            ...
            IF v_match_id IS NULL THEN
                ...
                IF v_candidates_count = 1 THEN
                    ...
                    v_match_level := 4; v_match_label := 'Monto redondo encontrado';
                ELSIF v_candidates_count > 1 THEN
                    v_fail_reason := 'Ambigüedad: se detectaron ' || v_candidates_count || ' registros internos con el mismo importe redondeado.';
                    
                    SELECT jsonb_agg(jsonb_build_object(
                        'mov_id', mt.id, 'entidad', e.razon_social, 'monto', mt.monto_total,
                        'reason', 'ambiguity', 'label', 'IDÉNTICO MONTO: Requiere que usted elija el correcto'
                    )) INTO v_suggestions
                    FROM (
    ...
    -- ============================================================
    -- CIERRE Y AUDITORÍA FINAL
    -- ============================================================
    v_result := jsonb_build_object(
        'status', 'success', 
        'matched_count', v_matched_count, 
        'admin_matched', v_admin_matched,
