-- Advanced Bank Reconciliation Engine v5.1 (FULL + CUIT Radar + Smart Assistant)
...
BEGIN
    ...
    LOOP
        ...
        IF v_inv_match.id IS NOT NULL AND NOT p_dry_run THEN
            ...
            SET monto_pendiente = GREATEST(0, COALESCE(monto_pendiente, monto_total) - abs(v_mov.monto_total)),
                estado = CASE WHEN (COALESCE(monto_pendiente, monto_total) - abs(v_mov.monto_total)) <= 2.0 THEN 'pagado' ELSE estado END
            WHERE id = v_inv_match.id;
        END IF;
    END LOOP;

    -- PHASE 2: Conciliación Bancaria V5.1 (Full Assistant + CUIT Radar)
    FOR v_trans IN 
    ...
    LOOP
        ...
        -- RADAR DE CUIT: Intento extraer 11 dígitos de la descripción o metadata
        v_extracted_cuit := COALESCE(
                                v_trans.metadata->>'cuit', 
                                (SELECT (regexp_matches(v_trans.descripcion, '(\d{11})'))[1] LIMIT 1)
                            );

        -- L1: Match por CUIT Radar (Muy Seguro)
        ...
        -- L3: 4 DÍGITOS + MONTO ENTERO
        IF v_match_id IS NULL THEN
            SELECT 
                CASE WHEN COUNT(DISTINCT mt.id) = 1 THEN (ARRAY_AGG(mt.id))[1] ELSE NULL END INTO v_match_id
        ...
        -- ASISTENTE: Generar sugerencias si falló todo lo anterior
        IF v_match_id IS NULL THEN
            -- S1: Diferencia de Monto (Misma Ref, Monto +/- 10%)
            SELECT json_agg(json_build_object(
                ...
                'reason', 'monto_diff',
                'label', 'Diferencia de monto (Posible ajuste/redondeo/parcial)'
            )) INTO v_suggestions
            FROM public.movimientos_tesoreria mt
            JOIN public.instrumentos_pago ip ON ip.movimiento_id = mt.id
            JOIN public.entidades e ON mt.entidad_id = e.id
            WHERE mt.organization_id = p_org_id
              AND RIGHT(REGEXP_REPLACE(COALESCE(ip.detalle_referencia, ''), '[^0-9]', '', 'g'), 4) = 
                  RIGHT(REGEXP_REPLACE(COALESCE(v_trans.descripcion, ''), '[^0-9]', '', 'g'), 4)
              AND ABS(mt.monto_total - ABS(v_trans.monto)) / NULLIF(ABS(v_trans.monto), 0) <= 0.10
              AND NOT EXISTS (SELECT 1 FROM public.transacciones WHERE movimiento_id = mt.id);

            -- S2: Monto Exacto, Referencia Diferente
            IF v_suggestions IS NULL OR json_array_length(v_suggestions) = 0 THEN
                SELECT json_agg(json_build_object(
                    'mov_id', mt.id,
                    'entidad', e.nombre,
                    'monto', mt.monto_total,
                    'reason', 'ref_diff',
                    'label', 'Monto exacto, pero la referencia no coincide'
                )) INTO v_suggestions
                FROM public.movimientos_tesoreria mt
                JOIN public.entidades e ON mt.entidad_id = e.id
                WHERE mt.organization_id = p_org_id
                  AND ABS(mt.monto_total - ABS(v_trans.monto)) <= 0.01
                  AND NOT EXISTS (SELECT 1 FROM public.transacciones WHERE movimiento_id = mt.id);
            END IF;
        END IF;
        ...
        IF v_match_id IS NOT NULL THEN
            ...
            IF NOT p_dry_run THEN
                UPDATE public.transacciones SET movimiento_id = v_match_id, estado = 'conciliado', monto_usado = ABS(monto),
                    metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('match_level', v_match_level, 'method', 'v5.1_full')
                WHERE id = v_trans.id;
                UPDATE public.instrumentos_pago SET estado = 'acreditado' WHERE movimiento_id = v_match_id;
            END IF;
        ELSE
            -- Guardar sugerencias para el Frontend
            IF v_suggestions IS NOT NULL AND json_array_length(v_suggestions) > 0 AND NOT p_dry_run THEN
                UPDATE public.transacciones 
                SET metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object('suggestions', v_suggestions)
                WHERE id = v_trans.id;
            END IF;
        END IF;
    END LOOP;
    ...
    IF NOT p_dry_run THEN
        ...
        VALUES (p_org_id, 'auto_rpc_v5.1_full', v_total_read, v_matched_count, v_result);
    END IF;
