...
export function ColumnMapper({ file, onMappingComplete, onCancel, importId, initialData }: ColumnMapperProps) {
    ...
    return (
        <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-6xl mx-auto flex flex-col max-h-[90vh] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-t border-gray-800 bg-gray-900/80 backdrop-blur-md flex justify-end gap-3 sticky bottom-0">
                <button
                    onClick={onCancel}
                ...
                >
                    {importId ? 'Re-procesar Archivo' : 'Procesar Archivo'} <ArrowRight className="w-4 h-4" />
                </button>
            </div>
        </div>
    )
